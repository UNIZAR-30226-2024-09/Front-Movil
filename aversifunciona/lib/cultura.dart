...
class cultura extends StatefulWidget {
  @override
  _cultura_State createState() => _cultura_State();
}

class _cultura_State extends State<cultura> {
  ...
  Future<void> filtrarPodcasts() async {
    try {
      final response = await http.post(
        ...
        body: jsonEncode({'genero': 'Cultura'}),
      );
      ...
      if (response.statusCode == 200) {
        ...
        final List<Map<String, dynamic>> podcastsAux = podcastsData.map((
            podcast) {
          return {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Row(
          children: [
            Text('Buscar cultura', style: TextStyle(color: Colors.white)),
          ],
        ),
        backgroundColor: Colors.black,
            ...
            title: TextButton(
              onPressed: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                      builder: (context) =>
                          PantallaPodcast(podcastId: podcasts[index]['id'],
                              podcastName: podcasts[index]['nombre'])
                  ),
                );
              },
              child: Row(
                children: [
                  FutureBuilder<Uint8List>(
                    future: _fetchImageFromUrl('${Env
                        .URL_PREFIX}/imagenPodcast/${podcasts[index]['id']}/'),
                  ...
                  Text(podcasts[index]['nombre'], style: const TextStyle(
